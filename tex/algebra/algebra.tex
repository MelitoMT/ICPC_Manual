...
\\\\
En realidad, podemos ver este problema desde otro enfoque. Buscaremos un polinomio tal que $p_k(n)=\sum_{i=1}^{n} i^k$ (la misma función de $f$ del enunciado), lo primero que queremos saber es qué grado tiene el polinomio para saber cuántos puntos necesitamos, esto lo haremos por inducción, nótese que si $k=1$ por lo visto antes $p_1(n)=\frac{n(n+1)}{2}$, es decir, es un polinomio de grado $k+1=2$, supongamos entonces que $p_{k-1}(n)$ tiene grado $k$ y consideremos ahora $p_k(n)$. Recordemos la identidad telescópica
\[
(n+1)^{k+1} - 1 = \sum_{i=1}^{n}\left[(i+1)^{k+1}-i^{k+1}\right].
\]
Expandiendo $(i+1)^{k+1}$ por el binomio de Newton y sumando en $i$, obtenemos
\[
(n+1)^{k+1}-1 = (k+1)\,p_k(n) + \sum_{j=0}^{k-1}\binom{k+1}{j}p_j(n).
\]
Por hipótesis de inducción, cada $p_j(n)$ con $j<k$ tiene grado $j+1\le k$, y $(n+1)^{k+1}$ tiene grado $k+1$. Como ningún otro término aporta grado $k+1$, concluimos que $p_k(n)$ es un polinomio de grado $k+1$.
\\\\
De lo anterior concluimos que necesitamos $k+2$ puntos por los que pase el polinomio $p_k$ para evaluarlo de forma eficiente (recordemos $p_k = f$). Estos podemos encontrarlos fácilmente, pues $f(0)=\sum_{i=1}^{0} i^k = 0$ (suma vacía, ya que el límite superior es menor al inferior) y de ahí $f(x) = f(x-1) + x^k$. Tras esto ocuparemos la interpolación vista arriba para calcular $f(n)$, de forma que la implementación nos quedaría
\begin{lstlisting}[language=C++]
const ll mod = 1e9 + 7;
