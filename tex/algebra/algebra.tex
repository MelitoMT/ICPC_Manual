...
\end{lstlisting}
Este procedimiento tiene una complejidad $O(nk)$, es recomendable por simplicidad usarse en casos donde $nk<1e7$ para evitar TLE (Time Limit Exceeded).
...
$n^2$ entradas del producto como una suma de $n$ productos, dando una complejidad $O(n^3)$). En realidad podemos multiplicar la matriz cuadrada por si misma $k-n$ veces usando exponenciación rápida\footnote{Ver sección de exponenciación rápida}, de esta forma obtenemos el $k$ ésimo término en tiempo $O(n^3log(k-n))$.
...
\\\\El razonamiento detrás de por qué esto es cierto es sencillo. Supongamos que no se cumple, es decir, que hay dos polinomios diferentes $p$ y $q$ de grado menor o igual a $n$ que pasan por todos los puntos dados. Entonces tendremos que $p-q$ es un polinomio de grado $\leq n$ que tiene $n+1$ raíces (los $x_i$), pero sabemos que un polinomio no nulo de grado $\leq n$ tiene a lo más $n$ raíces \footnote{https://www.matem.unam.mx/~max/AS2/N6.pdf}, de donde tenemos que debe ser nulo, es decir $p-q=0$, por lo que $p=q$.
\\\\
...
\\\\Nótese que en el caso de no tener módulo podemos simplemente precalcular $(x-1)(x-2)\cdots (x-n)$ y dividir entre $(x-i)$ cuando necesitemos calcularlo para un $i$.
...
\textbf{Ejercicio} Optimizar la interpolación de Lagrange como lo hicimos cuando los puntos son $1, 2, 3, \dots, n$, para cualquier progresión aritmética.
\\
