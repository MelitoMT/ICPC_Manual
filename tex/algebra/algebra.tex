...
\textbf{Ejercicio}: Si en lugar de $1 + 2 + 3 + \cdots + n$ tuviéramos $1^2 + 2^2 + 3^3 + \cdots + n^2$ cuál sería el resultado?
...
esto significa que $R_x$ depende de los $n$ términos anteriores, es decir es de orden $n$ y cada término puede tener una constante $C_i$ que le multiplica. Por ejemplo, la recurrencia de Fibonacci es de orden $2$ y la constante en cada uno de los términos es $1$.
...
Se le llama programación dinámica (DP por sus siglas en inglés) cuando memorizamos datos que nos ayuden a evitar hacer dobles cálculos, por ejemplo en Fibonacci podemos almacenar los términos que vayamos obteniendo y así cuando lleguemos al término $n$ ya tendremos calculado $n-1$ y $n-2$ y no tendremos que volver a calcularlo. En el caso de una recurrencia lineal general la forma de implementarlo con DP es bastante simple, es como hacer fuerza bruta, pero memorizando lo que ya calculamos, es decir
...
De la solución con programación dinámica notemos que para cada paso solo nos importa tener la información de los $n$ últimos términos y todos los demás, podemos olvidarlos. Si guardamos los $n$ últimos elementos en una matriz columna, podemos aplicarle una transformación lineal para obtener los siguientes elementos, es decir si originalmente teníamos los elementos $\{R_1,R_2,\cdots,R_n\}$, después de la transformación lineal tendremos los elementos $\{R_{2},R_{3},\cdots,R_{n+1}\}$, la transformación lineal es relativamente sencilla y consiste en multiplicar por una matriz cuadrada como sigue
...
\\\\El razonamiento detrás de por qué esto es cierto es sencillo, supongamos que no se cumple, es decir, que hay dos polinomios diferentes $p$ y $q$ de grado $\leq n$ que pasan por todos los puntos dados. Entonces tendremos que $p-q$ es un polinomio de grado $\leq n$ que tiene $n+1$ raíces (los $x_i$), pero sabemos que un polinomio no nulo de grado $\leq n$ tiene a lo más $n$ raíces, de donde tenemos que debe ser nulo, es decir $p-q=0$, por lo que $p=q$.
\\\\
...
\] 
entonces al menos para el numerador podemos precalcular los prefijos, es decir guardamos en un vector $x-1$, $(x-1)(x-2)$, $(x-1)(x-2)(x-3)$, $\cdots$, $(x-1)(x-2)\cdots(x-n)$ y los sufijos, que es lo mismo pero del último elemento al primero, es decir $(x-n)$, $(x-n)(x-(n-1))$ y así hasta $(x-n)(x-(n-1))\cdots(x-1)$, de forma que cuando queramos hacer la multiplicación de arriba podamos hacerlo en tiempo $O(1)$ multiplicando el prefijo $(x-1)\cdots(x-(i-1))$ con el prefijo $(x-n)\cdots(x-(i+1))$.
...
=(i-1)!(-1)^{n-i}(n-i)!
\] 
para esto entonces nos interesara poder precalcular los inversos de los factoriales\footnote{Ver la sección de inversos con módulo} y así para cada $i$ también lo podemos calcular en $O(1)$.
...
\\\\Nuestro primer instinto probablemente sea calcular el resultado sumando uno a uno los términos de esta suma; sin embargo, aun si se pudiera calcular las potencias $i^k$ de forma constante, tendríamos que sumar $n$ términos, es decir en el peor caso $10^9$ operaciones, lo que nos daría como veredicto TLE. 
\\\\
...
\\\\De lo anterior concluimos que necesitemos $k+2$ puntos por los que pase el polinomio para determinarlo, estos podemos encontrarlos fácilmente, pues $f(0)=0$ y de ahí $f(x)=f(x-1)+x^k$. Traes esto ocuparemos la interpolación vista arriba para calcular $f(n)$, de forma que la implementación nos quedaría
\begin{lstlisting}[language=C++]
const ll mod = 1e9 + 7;
...
\begin{itemize}
     %Recurrence
    \item \href{https://cses.fi/problemset/task/1096}{Throwing Dice (CSES 1092)}: Contar formas de obtener suma $n$ lanzando un dado de seis caras, con recurrencia y exponenciación de matrices.
    %Binary Exponentiation
    \item \href{https://cses.fi/problemset/task/1724}{Graph Paths II (CSES 1724)}: Camino de peso mínimo con exactamente $k$ aristas en un grafo dirigido ponderado, vía exponenciación de matrices.
    %Lagrange interpolation
    \item \href{https://codeforces.com/contest/995/problem/F}{Cowmpany Cowmpensation (Codeforces 995F)}: Contar asignaciones de salarios válidas en un árbol usando DP e interpolación de Lagrange.
    %Fourier transofrmation
    \item \href{https://codeforces.com/group/DH4HPwQlLU/contest/689800/problem/A}{Proyecto Hali Mary $\filledstar$}: Localizar regiones de un genoma similares a una secuencia dada, con convolución (FFT).
    \item \href{https://codeforces.com/contest/993/problem/E}{Nikita and Order Statistics (Codeforces 993E)}: Contar pares ordenados con restricciones de orden en arreglos, usando FFT e inclusión-exclusión.
    %Fourier tranformation
    \item \href{https://codeforces.com/contest/1398/problem/G}{Running Competition (Codeforces 1398G)}: Maximizar la distancia de vuelta en una pista, contando diferencias de posiciones con FFT.
    \item  \href{https://codeforces.com/group/DH4HPwQlLU/contest/689800/problem/B}{Rumbo al Mundial $\filledstar$}: Contar configuraciones de semáforos cuya suma de tiempos supera un umbral, con funciones generadoras y FFT.
\end{itemize}
