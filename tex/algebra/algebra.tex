...
\end{tabular}
\caption{$\gamma \approx 0.5772$ es la constante de Euler-Mascheroni.}
\end{table}
...
\]
Lo primemro que nos interesara será poder evaluarlo en algún punto de forma eficiente, para esto usaremos \textbf{método de Horner}, que simplemente consiste en reescribir el polinomio como:
\[
p(x) = a_0 + x(a_1 + x(a_2 + \cdots + x(a_{n-1} + x \cdot a_n)\cdots))
\]
Es decir, se empezará a calcular desde el coeficiente de mayor grado y se va acumulando de adentro hacia afuera, la implementación la haremos con módulo ya que es más usada, sin embargo si no se necesita basta con quitar ''mod'' del código.
...
\\\\El razonamiento detrás de por qué es sencillo, supongamos que no se cumple, es decir, que hay dos polinomios diferentes $p$ y $q$ de grado $\leq n$ que pasan por todos los puntos dados. Entonces tendremos que $p-q$ es un polinomio de grado $\leq n$ que tiene $n+1$ raíces (los $x_i$), pero sabemos que un polinomio no nulo de grado $\leq n$ tiene a lo más $n$ raíces, de donde tenemos que tiene que ser nulo, es decir $p-q=0$, por lo que $p=q$.
\\\\
...
\\\\Para practicar el tema, resolvamos el siguiente problema, dados $n$ y $k$ tal que $0 \leq k \leq 10^6$ y $1 \leq n \leq 10^9$, calcular
\[
f(n)=\sum_{i=1}^{n} i^k
\]
Imprimir la respuesta modulo $10^9+7$.
