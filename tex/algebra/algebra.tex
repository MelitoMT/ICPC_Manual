...
La \textbf{Transformada Teórica de Números} (NTT, por sus siglas en inglés) es una variante de FFT que opera en aritmética modular en lugar de números complejos. La idea es reemplazar las raíces $n$-ésimas de la unidad $\omega = e^{2\pi i/n}$ por una raíz primitiva $n$-ésima de la unidad módulo un primo $p$, es decir, un elemento $g$ tal que $g^n \equiv 1 \pmod{p}$ y $g^k \not\equiv 1 \pmod{p}$ para $0 < k < n$. Esto es posible cuando $p$ es un primo de la forma $p = c \cdot 2^k + 1$ con $2^k \geq n$, lo que garantiza que existen raíces de la unidad de cualquier orden potencia de dos. El primo más común en competencia es $p = 998244353 = 119 \cdot 2^{23} + 1$. Al trabajar enteramente con enteros módulo $p$, NTT evita los errores de punto flotante y produce resultados exactos.
...
overflow.
\\\\Ver \href{https://cses.fi/paste/9daa72fef9f7cb2910720a3/}{código}.
        %Binary Exponentiation
        \item \href{https://cses.fi/problemset/task/1723}{Graph Paths I (CSES 1723)}
\\\\En este problema nos dan una gráfica dirigida, notemos que esta podemos expresarla por medio de una matriz de adyacencia $M$. Esta tendrá $M[i][j]=1$ si hay una arista de $i$ a $j$, de lo contrario, tendrá $0$. Podríamos pensar que este es un problema en el que necesitamos algún algoritmo de gráficas, pero la realidad es que podemos resolverlo con exponenciación de matrices. 
...
\]
notemos que $M^k$ suma para cada posible penultimo vértice $m$ en el camino de $i$ a $j$, el número de caminos de longitud $k-1$ de $i$ a $m$ multiplicado por $M[m][j]$ es decir, solo se cuentan los caminos si existe alguna arista $m$-$j$, pues de lo contrario no podría existir ese camino.
...
\\\\Ver \href{https://cses.fi/paste/9d5c79555ee5bd881072387/}{código}.
    \end{itemize}
\section{Problemas}
