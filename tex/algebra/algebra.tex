...
Las sumas no solo sirven para obtener la respuesta de problemas. También nos ayudan a evaluar complejidades, las identidades más comunes que se usan en programación competitiva son:
\begin{table}[H]
\centering
...
// Saltamos los primeros n terminos, que son las condiciones iniciales.
for (int i = n; i <= k; i++) {
    dp[i] = 0;

    // Cada termino anterior j contribuye con su constante c[j].
    for (int j = 1; j <= n; j++) {
        dp[i] += c[j] * dp[i - j];
    }
}
...
\begin{lstlisting}[language=C++]
long long evaluateHorner(const vector<long long>& coefficients, long long x) {
    long long result = 0;
...
long long sumOfPowers(int n, int k) {
    ...
    if (n <= k + 1) {
        long long result = 0;
        for (int i = 1; i <= n; i++) {
            result = (result + power(i, k)) % MOD;
        }
        return result;
    }

    vector<long long> prefix(k + 2);
    vector<long long> suffix(k + 2);
    vector<long long> fact(k + 2);
    vector<long long> invFact(k + 2);
    ...
    invFact[k + 1] = power(fact[k + 1], MOD - 2, MOD);
    for (int i = k; i >= 0; i--) {
    ...
    for (int i = 0; i <= k + 1; i++) {
        ...
        long long denominator = invFact[i] * invFact[k + 1 - i] % MOD;
        long long term = yValue * numerator % MOD * denominator % MOD;
...
\section{Transformada de Fourier rápida (FFT) y NTT\adv}
\advnote{Requiere polinomios (sección anterior) y, para NTT, aritmética modular e inversos (Cap.~6). Puede omitirse en una primera lectura.}
...
void fft(std::vector<Complex>& values, bool invert) {
    int n = values.size();
    ...
    for (int length = 2; length <= n; length <<= 1) {
        ...
        Complex rootLength(cosl(angle), sinl(angle));

        for (int i = 0; i < n; i += length) {
            Complex root(1);
            for (int j = 0; j < length / 2; j++) {
                Complex u = values[i + j];
                Complex v = values[i + j + length / 2] * root;
                values[i + j] = u + v;
...
void ntt(std::vector<long long>& values, bool invert) {
    int n = values.size();
    ...
    for (int length = 2; length <= n; length <<= 1) {
        long long root = invert
            ? power(PRIMITIVE_ROOT, MOD - 1 - (MOD - 1) / length, MOD)
            : power(PRIMITIVE_ROOT, (MOD - 1) / length, MOD);

        for (int i = 0; i < n; i += length) {
            long long currentRoot = 1;
            for (int j = 0; j < length / 2; j++) {
                long long u = values[i + j];
                long long v = values[i + j + length / 2] * currentRoot % MOD;
                values[i + j] = (u + v) % MOD;
                values[i + j + length / 2] = (u - v + MOD) % MOD;
    ...
    if (invert) {
        long long invN = power(n, MOD - 2, MOD);
        for (long long& value : values) {
            value = value * invN % MOD;
        }
    }
}
\end{lstlisting}
En la práctica, lo que se llama desde el problema es simplemente \texttt{multiplyNtt(a, b)} o \texttt{multiplyFft(a, b)},
que devuelve el vector de coeficientes del producto. Las funciones \texttt{ntt} y \texttt{fft}
son internas.
 
\begin{lstlisting}[language=C++]
std::vector<long long> multiplyNtt(std::vector<long long> a, std::vector<long long> b) {
...
std::vector<long long> multiplyFft(std::vector<long long> a, std::vector<long long> b) {
    int resultSize = a.size() + b.size() - 1;
    int n = 1;
    while (n < resultSize) {
        n <<= 1;
    ...
    std::vector<long long> result(resultSize);
...
\end{lstlisting}
\section{Ejercicios}
\begin{tcolorbox}[colback=green!5, colframe=codegreen!50, title=Nota]
  Las descripciones de los problemas están en los links de los títulos.
\end{tcolorbox}
...
overflow.
\\\\Ver \href{https://github.com/MelitoMT/ICPC_Manual/tree/master/tex/codes/editorial/1722.cpp}{código}.
