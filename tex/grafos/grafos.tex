Una gráfica $G = (V, E)$ consiste en un conjunto de \textbf{vértices} $V$ y un conjunto de \textbf{aristas} $E \subseteq \binom{V}{2}$. Asumimos familiaridad con las definiciones básicas: vecindad, grado, caminos, ciclos, conectividad. Esta sección se enfoca en resultados matemáticos que fundamentan el resto del capítulo y que raramente se ven en cursos de computación.
\\\\
Recordemos que hay dos representaciones estándar de una gráfica, las cuáles tienen complejidades muy distintas por lo que sirven para diferentes problemas. En general, la lista de adyacencia es preferible cuando $E \ll V^2$; la matriz cuando se necesita consultar aristas individuales en $O(1)$ o cuando $V$ es pequeño ($V \leq 500$) y se necesitan consultas en $O(1)$.
...
El algoritmo de Kosaraju encuentra todas las SCCs en $O(V + E)$ con dos DFS. El algoritmo se basa en una observación sobre el \textbf{grafo transpuesto} $G^T$ (el mismo grafo pero con todas las aristas invertidas). Las SCCs de $G$ y $G^T$ son \textbf{exactamente las mismas}, invertir las aristas no cambia qué nodos se alcanzan mutuamente dentro de una SCC. Pero el orden en que las SCCs se ``descubren'' sí cambia.
\\\\
El primer DFS lo hacemos en $G$ y anotamos el orden en que terminan (orden de salida). El
último nodo en terminar pertenece a la SCC que no tiene aristas entrantes desde otras SCCs.
\\\\
El segundo DFS lo haremos en $G^T$ procesando los nodos en orden inverso
al de salida. Cada DFS que se pueda iniciar desde un nodo no visitado
encontrará exactamente una SCC.
...
Este algoritmo funciona porque en el segundo DFS sobre $G^T$, empezamos
por el nodo con mayor tiempo de salida. Desde ese nodo en $G^T$ solo
podemos alcanzar nodos de su propia SCC porque las aristas que salían
de esa SCC en $G$ ahora entran a ella en $G^T$, y las aristas que
entraban ahora salen, por lo que los nodos alcanzables desde él en
$G^T$ forman su SCC.
...
\section{2-SAT}
Motivemos esta sección con este problema: \href{https://codeforces.com/problemset/problem/776/D}{The door problem}. Se nos dice que hay $n$ personas atrapadas en $n$ cuartos. Cada cuarto tiene una puerta que puede estar bloqueada o desbloqueada. Hay $m$ interrumptores, activar o desactivar el interrumptor cambia el estado de las puertas que controla. Cada puerta es controlada por dos interrumptores. ¿Es posible operar los
...
\\\\
Supongamos tenemos $n$ variables, cada una puede ser verdadera o falsa. Ahora, estas variables teinen restricciones, por ejemplo $x_1$ es verdadero o $x_2$ es verdadero. La pregunta es, ¿existe una asignación de verdadero/falso a todas las variables que satisfaga todas las restricciones?
\\\\
...
// Calcula el tamanio del subarbol de u en la componente actual
...
// Encuentra el centroide de la componente
...
// u es cualquier nodo en la componente actual
// p es el padre en el arbol de centroide (-1 para la raiz)
void build(int u, int p) {
    int total = calcSz(u, -1);       // tamanio de esta componente
    int c = centroid(u, -1, total);   // encontrar su centroide
    cpar[c] = p;                      // c es hijo de p en el arbol de centroide
    removed[c] = true;                // marcar c como procesado
...
\section{Ancestro común más bajo (LCA)}
...
// Precalcular ancestros con DFS desde la raiz
...
int lca(int u, int v) {
    ...
    // revisar si ya encontramos el LCA
    if (u == v) return u;
...
\end{definition}
Un teorema muy importante es el \textbf{Teorema de Flujo Máximo y Corte Mínimo} que nos dice que el valor del flujo máximo de $s$ a $t$ es igual a la capacidad del corte mínimo $s$-$t$\footnote{Ver \href{https://www.cs.toronto.edu/~lalla/373s16/notes/MFMC.pdf}{demostración}}.
\\\\
...
El \textbf{teorema de König} nos dice que en una gráfica bipartita el matching máximo es igual a la cobertura mínima de vértices\footnote{Ver \href{https://www.math.nagoya-u.ac.jp/~richard/teaching/s2024/SML_Li_2.pdf}{demostración}}.
\\\\
...
$i$ a la tarea $j$, minimizar el costo total. La mejor referencia que he encontrado para este algoritmo es este \href{https://www.topcoder.com/thrive/articles/Assignment%20Problem%20and%20Hungarian%20Algorithm}{blog}.
...
Ver \href{https://codeforces.com/contest/808/submission/273807670}{código}.
...
\end{itemize}
 ...
 Ver \href{https://github.com/MelitoMT/ICPC_Manual/tree/master/tex/codes/editorial/qtree2.cpp}{código}.

\section{Problemas}
\begin{itemize}
  \item[9.11.1] \href{https://cses.fi/problemset/task/2079}{Finding a Centroid (Codeforces)}
  \item[9.11.2] \href{https://codeforces.com/group/DH4HPwQlLU/contest/689800/problem/I}{Zuko vs Aang (Autoría)}
