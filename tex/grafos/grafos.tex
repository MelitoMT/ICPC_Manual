Una gráfica $G = (V, E)$ consiste en un conjunto de \textbf{vértices} $V$ y un conjunto de \textbf{aristas} $E \subseteq \binom{V}{2}$. Recordemos brevemente las nociones elementales que usaremos. Dos vértices $u$ y $v$ son \textbf{vecinos} (o adyacentes) si $\{u,v\}\in E$, y el \textbf{grado} $\deg(v)$ de un vértice es su número de vecinos. Un \textbf{camino} de $u$ a $v$ es una sucesión de vértices $u=v_0, v_1, \dots, v_k=v$ tal que $\{v_{i-1},v_i\}\in E$ para cada $i$; su \textbf{longitud} es $k$, el número de aristas que recorre. Un camino es \textbf{simple} si no repite vértices. Un \textbf{ciclo} es un camino de longitud al menos $3$ que empieza y termina en el mismo vértice y no repite ningún otro vértice. Una gráfica es \textbf{conexa} si entre cualquier par de vértices existe un camino. En una gráfica \textbf{dirigida} las aristas son pares ordenados $(u,v)$ y los caminos deben respetar su dirección. Esta sección se enfoca en resultados matemáticos que fundamentan el resto del capítulo y que raramente se ven en cursos de computación.
\\\\
Existen dos representaciones estándar de una gráfica, con complejidades muy distintas:
\begin{itemize}
    \item \textbf{Lista de adyacencia}: conviene cuando la gráfica es rala, es decir, cuando el número de aristas es mucho menos que el máximo posible $|E| \ll |V|^2$.
    \item \textbf{Matriz de adyacencia}: conviene cuando la gráfica es densa, cuando se necesitan consultas de adyacencia de aristas en $O(1)$, o cuando $V$ es pequeño ($|V| \leq 500$) y ese costo de espacio es aceptable.
\end{itemize}
...
Como corolario, tenemos que, en toda gráfica, el número de vértices de grado impar es par.
\\\\
$Demostración$.
...
\\\\
Un resultado análogo de este corolario para digráficas tiene consecuencias sorprendentes en el ICPC. Por ejemplo: en un torneo (una gráfica donde entre cada par de vértices existe una única flecha), la suma de grados de salida es $\binom{n}{2}$; argumentos de paridad sobre los grados pueden determinar si cierta configuración es posible sin necesidad de construirla.

\section{Caminos y circuitos de Euler}

Nos preguntamos ahora si es posible recorrer una gráfica pasando por cada arista exactamente una vez. La respuesta resulta ser una condición muy simple sobre los grados, lo que hace de este uno de los pocos temas donde reconocer el patrón basta para resolver el problema. Empecemos por el ejemplo histórico que dio origen a la teoría de gráficas.
...
Para modelarlo, Euler representó cada zona de tierra con un punto y cada puente con una línea entre las dos zonas que conecta. En la figura, a la izquierda aparece un esquema de la ciudad, con el río, las zonas de tierra $A$, $B$, $C$, $D$ y los puentes numerados del 1 al 7, y a la derecha la gráfica que se obtiene: un vértice por zona y una arista por puente.
\begin{figure}[H]
\centering
...
\begin{tikzpicture}[scale=0.58]
  \begin{scope}
    \clip (0,-2.4) rectangle (8.4,2.4);
    % tierra
    \fill[green!15] (0,-2.4) rectangle (8.4,2.4);
    % rio
    \fill[blue!25] (-1,-0.7) -- (-1,0.7) -- (5.2,0.7) -- (9.2,3.7) -- (9.8,3.0)
      -- (5.8,0) -- (9.8,-3.0) -- (9.2,-3.7) -- (5.2,-0.7) -- cycle;
    % puentes
    \begin{scope}[line width=2.2pt, brown!70!black]
      \draw (2.2,0.2) -- (2.2,0.9);   \draw (3.8,0.2) -- (3.8,0.9);
      \draw (2.2,-0.2) -- (2.2,-0.9); \draw (3.8,-0.2) -- (3.8,-0.9);
      \draw (4.3,0) -- (6.0,0);
      \draw (6.75,0.45) -- (5.85,1.5);
      \draw (6.75,-0.45) -- (5.85,-1.5);
    \end{scope}
    % isla
    \fill[green!15] (3,0) ellipse (1.6 and 0.4);
    \draw[green!40!black] (3,0) ellipse (1.6 and 0.4);
  \end{scope}
  \draw (0,-2.4) rectangle (8.4,2.4);
  % etiquetas
  \node at (3,0) {$A$};
  \node at (1.2,1.7) {$B$};
  \node at (1.2,-1.7) {$C$};
  ...
  \begin{scope}[font=\scriptsize]
    \node[above] at (2.2,0.9) {1};  \node[above] at (3.8,0.9) {2};
    \node[below] at (2.2,-0.9) {3}; \node[below] at (3.8,-0.9) {4};
    \node[above] at (5.15,0) {5};
    \node[above right] at (6.3,1.0) {6};
    \node[below right] at (6.3,-1.0) {7};
  \end{scope}
\end{tikzpicture}
\end{minipage}\hfill
\begin{minipage}{0.49\linewidth}
...
\begin{tikzpicture}[
  ...
  \draw (A) to[bend right=20] node[draw=none,right,font=\scriptsize]{2}(B);
  \draw (A) to[bend left=20]  node[draw=none,right,font=\scriptsize]{3}(C);
  \draw (A) to[bend right=20] node[draw=none,below,font=\scriptsize]{4}(C);
  \draw (A) --                node[draw=none,below,font=\scriptsize]{5}(D);
...
\begin{itemize}
    ...
    \item $G$ es \textbf{euleriana} si tiene un circuito euleriano.
\end{itemize}
\end{definition}
 
Sea $G$ conexa~\cite{illinois_euler}. Entonces:
\begin{itemize}
    \item $G$ tiene un \textbf{circuito euleriano} si y solo si todo
          vértice tiene grado par;
    \item $G$ tiene un \textbf{camino euleriano} que no es circuito si y solo
...
de forma no obvia. El patrón general es construir una gráfica donde lo
que se quiere recorrer son las \textbf{aristas}, no los vértices. Imaginemos que nos dan un conjunto de palabras y nos preguntan, ¿es posible ordenarlas en una sucesión tal que la última letra de cada palabra sea la primera de la siguiente? Podemos modelar cada letra como un vértice, cada palabra como una arista dirigida de su primera letra a su última. La
sucesión existe si y solo si la gráfica tiene un camino euleriano.
\section{Componentes fuertemente conexas}
En una gráfica \textbf{no dirigida}, la conectividad es simple, hay un
camino entre dos vértices o no. Pero en una gráfica \textbf{dirigida} la
...
es un subconjunto máximo de vértices $C$ tal que para todo par
$u, v \in C$ existe un camino de $u$ a $v$ y un camino de $v$ a $u$.
\end{definition}
\subsection{Algoritmo de Kosaraju}
El algoritmo de Kosaraju encuentra todas las SCCs en $O(V + E)$ con dos DFS. Se apoya en la \textbf{gráfica transpuesta} $G^T$ (mismos vértices, aristas invertidas). Las SCCs de $G$ y $G^T$ son las mismas, aunque cambia el orden en que conviene descubrirlas.
\\\\
El procedimiento tiene dos pasos:
\begin{enumerate}
    \item \textbf{DFS en $G$.} Al terminar de explorar cada vértice, lo añadimos al final de una lista \texttt{order}. Un vértice se registra tarde cuando aún quedaban descendientes por visitar; por eso el último de la lista pertenece a una SCC sin aristas entrantes desde otras componentes.
    \item \textbf{DFS en $G^T$, en orden inverso.} Recorremos \texttt{order} de atrás hacia adelante. Cada vez que encontramos un vértice sin componente asignada, lanzamos un DFS en $G^T$ desde ahí. Ese DFS descubre \emph{exactamente} una SCC: en $G^T$ las aristas que salían hacia otras componentes se volvieron entradas, así que no podemos salir de la componente actual.
\end{enumerate}
...
En el ejemplo de la figura las SCCs son $\{a,b,c\}$ y $\{d,e,f\}$. Veamos el algoritmo paso a paso suponiendo que los vértices se numeran $a=0,b=1,c=2,d=3,e=4,f=5$ y que el primer DFS recorre $G$ en ese orden.

\textbf{Paso 1 (DFS en $G$).} Empezamos en $a$: visitamos $a \to b \to c$; desde $c$ seguimos a $d$, luego $d \to e \to f$, y al retroceder registramos cada vértice al terminar:
\[
...
\begin{itemize}
    ...
    \item Los vértices $b$ y $c$ ya tienen componente; el siguiente libre es $d$. Desde $d$ en $G^T$ alcanzamos $e \to f \to d$ y también los predecesores $b,c$ (ya visitados). Solo se agrega $\{d,e,f\}$ con \texttt{comp} $= 1$.
\end{itemize}
...
int comp[MAXN];                     // SCC de cada vertice
...
condensación}, una gráfica donde cada vértice representa una SCC y hay una arista de la SCC $i$ a la SCC $j$ si existe alguna arista en $G$ de un vértice en $i$ a un vértice en $j$. Esta gráfica siempre es un DAG (gráfica dirigida acíclica)~\cite{cpalgos_scc}.
...
\begin{itemize}
    ...
    \item $(\neg x_1 \lor x_3)$: si $x_1$ es verdadero, entonces $x_3$ debe ser verdadero.
...
Una fórmula 2-SAT es satisfacible si y solo si en la gráfica de implicaciones ninguna variable $x_i$ está en la misma SCC que su negación $\neg x_i$. Si la fórmula es satisfacible, la asignación se obtiene directamente de los números de SCC que asigna Kosaraju:
...
\\\\Si la puerta está abierta, necesitamos que ambos interruptores que la controlan se activen (sean verdaderos) o que ninguno lo haga (sean falsos). 
...
En una gráfica conexa $G$:
\begin{itemize}
    \item Una arista es un \textbf{puente} si al eliminarla la gráfica
          se desconecta.
    \item Un vértice es un \textbf{punto de articulación} si al
          eliminarlo (junto con sus aristas incidentes) la gráfica se
          desconecta.
\end{itemize}
\end{definition}

En la siguiente gráfica, la arista roja $d$-$e$ es un puente. Los vértices $d$ y $e$ son puntos de articulación porque al eliminarlos desconectan la gráfica.
\begin{figure}[H]
\centering
\begin{tikzpicture}[
  every node/.style={circle,draw,minimum size=0.65cm,font=\small},
...
aristas que no forman parte de este árbol conectan un vértice con algún \textbf{ancestro} suyo les llamaremos aristas hacia atrás.
\\\\
La idea para construir un algoritmo más eficiente es ver que un vértice $U$ es punto de articulación si tiene algún hijo $V$ tal que
\textbf{ningún vértice del subárbol de $V$ puede llegar a un ancestro de
...
\end{tikzpicture}
\end{figure}
Para que una arista $(uv)$  con $v$ hijo de $u$ n el DFS sea un puente, se debe cumplir que $\texttt{low}[v] > \texttt{disc}[u]$~\cite{cpalgos_bridges}. Ya que eso significa que el subárbol de $v$ no puede alcanzar a $u$ sin pasar por la arista $(u,v)$. Dicho esto, es lógico que la condición para que un vértice $u$ sea punto de articulación sea~\cite{cf_bridges}:
\begin{itemize}
...
\end{lstlisting}
\textbf{Ejercicio.} Implementar la solución completa de \href{https://onlinejudge.org/external/3/315.pdf}{Network} usando
este algoritmo.
 
\section{Árboles}
 
Un \textbf{árbol} es una gráfica conexa (es decir, todos sus vértices están conectados por un camino) y acíclico (sin ciclos). Esta definición es solo una de varias caracterizaciones equivalentes, todas igualmente válidas como definición y útiles en distintos contextos.
 
Para una gráfica $G$ con $n$ vértices, las siguientes afirmaciones son equivalentes:
\begin{enumerate}
    \item $G$ es conexa y acíclica;
    \item $G$ es conexa y tiene exactamente $n-1$ aristas;
    \item Entre todo par de vértices existe exactamente un camino simple (no repite vértices).
\end{enumerate}
...
Para motivar este capítulo de árboles, veamos el problema \href{https://codeforces.com/contest/342/problem/E}{Xenia and Tree}, en este nos dan un árbol con $n$ vértice. El vértice $1$ está pintado de rojo y los demás de azul. Nos pedirán consultas de dos tipos:
\begin{itemize}
    \item[1.] Nos dan un número $a$ que corresponde a un vértice azul y tenemos que pintarlo de rojo.
    \item[2.] Nos dan un número $b$ que corresponde a un vértice del árbol y tenemos que imprimir la distancia entre $b$ y el vértice rojo más cercano.
\end{itemize}
Nótese que calcular la distancia a cada uno de los vértices rojos cada que nos piden una consulta no es eficiente y en este problema que nos pueden pedir $10^5$ consultas en un árbol de $10^5$ vértices, esta solución ingenua resultaría en $TLE$. Veremos algunas herramientas que nos ayudarían a resolver este problema.
...
\subsubsection*{Descomposición por centroides}

Tomemos un árbol más grande que el anterior y tomemos su centroide $c$. Al eliminarlo, el árbol se divide en varias componentes, cada una de tamaño $\leq \lfloor n/2 \rfloor$. Nótese que entonces cualquier camino de un vértice $v$ en una componente a otro $u$ en otra componente, este pasa por $c$. Por tanto, la distancia entre $u$ y $v$ podemos sumar la distancia entre $c$ y $v$ y la distancia entre $c$ y $u$.
...
\end{tikzpicture}
\end{figure}
 La observación anterior solo resuelve caminos entre componentes distintas. Pero los vértices rojos también pueden estar en la misma componente que $v$. La solución es aplicar el mismo argumento recursivamente, cada componente tiene su propio centroide, y todo camino dentro de ella que no pasa por ese centroide, pasará por el centroide de alguna subcomponente más pequeña.

 Esto define el \textbf{árbol de centroide}: tomamos el centroide $c$ del árbol completo como raíz, y aplicamos el mismo proceso recursivamente en cada componente que queda al eliminarlo, y los centroides de esas componentes se vuelven hijos de $c$ en el árbol de centroide.
...
Notemos entonces que para cualquier par de vértices $u$ y $v$, existe un ancestro común más bajo (LCA) $w$ en el árbol de centroide tal que el camino entre los vértices $u$ y $v$ en el árbol original pasa por $w$; este es el primer ancestro común de $u$ y $v$ en el árbol de centroide. Por ejemplo, para $g$ y $e$ en el árbol anterior, este es $c$, es decir el primer ancestro que comparten. En caso de $e$ y $a$, es $b$.

%NOTA (buscar algun recuadro de color para estas cosas)
Una propiedad importante es que el árbol de centroide tiene profundidad $O(\log n)$, ya que cada vez que bajamos un nivel, el subárbol tiene tamaño $\leq \lfloor n/2 \rfloor$ (por la propiedad del centroide). Tras $k$ niveles el tamaño es $\leq n/2^k$. La recursión termina cuando el tamaño llega a 1, es decir, la profundidad es a lo más $\lceil \log_2 n \rceil$, esto es relevante para calcular la compleljidad del algortimo.

Volviendo al problema, con el árbol de centroide construido, mantenemos para cada vértice $c$:
\[
\texttt{minDist}[c] = \min_{u \text{ rojo}} \text{dist}(c,\, u)
\]
la distancia mínima desde $c$ a cualquier vértice rojo en el subárbol \emph{original} sobre el que $c$ fue centroide. Inicialmente $\texttt{minDist}[c] = \infty$ para todo $c$. Ahora, cada que nos pidan una consulta del tipo $1$, en la que nos pidan pintar un vértice $v$ de rojo, para todo vértice $c$ ancestro de $v$ en el árbol de centroide, debemos actualizar:
\[
...
\medskip
Para el segundo tipo de consulta notemos que para cualquier vértice rojo $u$, existe un ancestro $c$ de $v$ en el árbol de centroide tal que el camino de $v$ a $u$ pasa por $c$.
...
\\\\
La forma más directa es hacer un BFS o DFS desde $c$ cada vez, pero eso cuesta $O(n)$ por llamada. Con $O(\log n)$ ancestros por consulta y $m$ consultas, el costo total sería $O(mn \log n)$ que para $n = m = 10^5$ da $\sim 10^{11}$ operaciones. Demasiado lento.
\\\\
...
bool removed[MAXN];    // true si el vertice ya fue usado como centroide
...
// u: cualquier vertice de la componente actual.
...
Dado un árbol con raíz, el \textbf{ancestro común más bajo} (LCA) de dos vértices $u$ y $v$ es el vértice de mayor profundidad que es ancestro de ambos~\cite{youtube_lca}.
\begin{figure}[H]
\centering
\begin{tikzpicture}[
  every node/.style={circle, draw, minimum size=0.65cm, font=\small},
...
\end{tikzpicture}
\end{figure}
La solución más directa que podríamos pensar es ver qué vértice está más bajo o profundo y llevarlo al mismo nivel que el otro subiendo de a un paso, luego subir ambos juntos hasta que coincidan. Por ejemplo para $c$ y $u$ en la figura de arriba, primero subiríamos $u$ a su ancestro $d$ y ya que ambos $d$ y $c$ estpan en el mismo nivel, los subiriamos a su ancestro $a$ que como coincide, diríamos que este es su $LCA$.
\\\\
Imaginemos que tenemos un árbol en forma de cadena ($1 - 2 - 3 - \cdots - n$), calcular $\text{LCA}(1, n)$ cuesta $O(n)$ pasos. Con $m$ consultas, el costo total es $O(mn)$, lo que nos deja con el mismo problema que teníamos antes, necesitamos algo más eficiente. En lugar de subir de $1$ en $1$, ¿podríamos subir de a $2^k$ niveles de una vez?
\\\\
 Para subir $d$ niveles desde un vértice $v$, podemos descomponer $d$ en potencias de 2 (como en la representación binaria) y dar saltos de tamaño $1, 2, 4, 8, \ldots$ según corresponda. Si $d = 13 = 8 + 4 + 1$, damos un salto de 8, luego uno de 4, luego uno de 1, tres saltos en lugar de 13.
\\\\
Entonces necesitamos precalcular, para cada vértice $v$ y cada $k$:
\[
...
Como estamos subiendo en potencias de $2$, $\lfloor \log_2 n \rfloor + 1$ niveles basta para cubrir cualquier distancia hasta $n$. En la siguiente imagen, vemos como para el arbol que es una cadena que mencionamos antes, esta forma de encontrar el ancestro sí resulta eficiente. Para el vértice $v$, los ancestros a distancia $2^0=1$, $2^1=2$ y $2^2=4$ se precalculan. Para subir 5 niveles, basta un salto de 4 y un salto de 1, solo 2 operaciones.
...
Lo que haremos entonces para calcular el $LCA$  de dos vértices $u$ y $v$ será primero igualar sus profundidades. Si $\text{prof}(u) > \text{prof}(v)$, necesitamos subir $u$ exactamente $d = \text{prof}(u) - \text{prof}(v)$ niveles. Descomponemos $d$ en binario y usamos los saltos correspondientes.
\\\\
Una vez que coinciden, revisamos si $u = v$, ai se cumple, ese es el LCA. De lo contrario, subimos ambos vértices simultáneamente, usando el salto más grande posible tal que $\texttt{up}[u][k] \neq \texttt{up}[v][k]$ (es decir, que no hayan convergido todavía). Después de este proceso, $u$ y $v$ son hijos del LCA.
...
int lca(int u, int v) {
    ...
    // Subir ambos vertices juntos hasta quedar justo debajo del LCA.
...
Notemos que para calcular la distancia entre dos vértices $u$ y $v$, el camino de $u$ a $v$ sube desde $u$ hasta el LCA $d$ y luego baja hasta $v$. La distancia total es la suma de los dos tramos, que se simplifica a $\text{prof}(u)+\text{prof}(v)-2\cdot\text{prof}(\text{LCA})$.
\begin{figure}[H]
\centering
\begin{tikzpicture}[
  every node/.style={circle, draw, minimum size=0.7cm, font=\small},
...
\begin{enumerate}
    ...
    \item Para cada consulta de tipo 1 (pintar $v$), subir $O(\log n)$ ancestros en el árbol de centroide, calcular \texttt{dist} en $O(\log n)$ en cada uno. $O(\log^2 n)$.
    \item Para cada consulta de tipo 2 (distancia mínima desde $v$), igual que el tipo 1, subir $O(\log n)$ ancestros en el árbol de centroide calculando \texttt{dist} en $O(\log n)$ en cada uno. $O(\log^2 n)$.
\end{enumerate}
...
\advnote{Tema avanzado de gráficas: incluye Ford--Fulkerson, Dinic, emparejamiento y los teoremas de König y Hall. Puede omitirse en una primera lectura.}
...
\end{tikzpicture}
\end{figure}
Mirando la gráfica residual, buscamos otra ruta de $s$ a $t$. Vemos que $s\to v\to t$ no funciona porque $v\to t$ está saturado (capacidad
residual 0).
\\\\
...
\subsection{Dinic} 
Dinic resuelve el problema de la cantidad de iteraciones organizando el trabajo por fases. En cada fase, construye la \textbf{gráfica por niveles} a partir de $s$ mediante BFS, y luego calcula un \textbf{flujo bloqueante} sobre ella, es decir, un flujo tal que ya no queda ningún camino $s$-$t$ disponible dentro de la gráfica por niveles antes de repetir el proceso con niveles más largos. Veamos con el siguiente ejemplo cómo funciona:

Lo primero que haremos será correr un $BFS$ desde $s$ para calcular las distancias de $s$ a los vértices y asignar niveles que representaremos con $\ell$. Véase que $t$ es alcanzable desde $s$ por un camino de longitud $3$ (por $c\to t$). Existe también un camino de longitud $4$ hasta $t$, pero como la distancia toma el camino más corto,  $\ell(t)=3$. Para que una arista $(u,v)$ entre en la gráfica por niveles debe cumplir una regla: $\ell(v) = \ell(u)+1$. La arista $e\to t$ no cumple, pues ($\ell(t)=3 \neq \ell(e)+1=4$) por lo que se ignora.
...
\end{tikzpicture}
\end{figure}
Notemos que el único camino disponible es $s\to a\to c\to t$, el flujo que salga de este será entonces $\min(2,2,1) = 1$. La arista $c\to t$ queda saturada.
\\\\
...
\end{tikzpicture}
\end{figure}
Lo que se hace a continuación es, por medio de DFS, encontrar caminos $s$-$t$ dentro de esta gráfica por niveles (de longitud 4) y enviar flujo por ellos, repitiendo hasta que ya no quede ningún camino disponible, es decir, hasta obtener un flujo bloqueante.
...
Esto confirma que, en esta fase, alcanzamos un flujo bloqueante, es decir, ya no hay ningún camino $s$-$t$ dentro de la gráfica por niveles actual, aunque en la red original (sin restricción de niveles) sí podría quedar flujo residual disponible.
...
struct Dinic {
    ...
    // Construye el grafica de niveles en la red residual.
    bool bfs() {
        while (!q.empty()) {
            int u = q.front();
            q.pop();
...
Ya sabemos calcular el flujo máximo, pero su verdadera utilidad en concurso viene de que ese mismo número responde una pregunta aparentemente distinta: cuál es la forma más barata de desconectar la fuente del sumidero. Esa equivalencia es la que permite modelar como flujo problemas que hablan de eliminar elementos o de romper conflictos, como veremos en los ejercicios.
...
\end{definition}
Un teorema muy importante es el \textbf{Teorema de Flujo Máximo y Corte Mínimo}~\cite{toronto_mfmc}, que nos dice que el valor del flujo máximo de $s$ a $t$ es igual a la capacidad del corte mínimo $s$-$t$.
\\\\
Después de correr Dinic, si hacemos BFS en el gráfica residual desde $s$.
Los vértices alcanzables forman $S$ y los no alcanzables $T$. Los cables
saturados de $S$ a $T$ forman el corte mínimo.
\\\\
Este teorema tiene consecuencias directas que usaremos en la sección siguiente de emparejamiento.
\\\\
Una aplicación bonita que podemos darle es encontrar \textbf{caminos ajenos por aristas}, es decir, el número máximo de caminos de $s$ a
...
Nótese entonces que queremos seleccionar un conjunto de vértices $S$ de forma que cualquier arista tenga al menos un extremo en $S$, a esto se le llama \textbf{cubierta de vértices}. Queremos entonces la cubierta de menor tamaño, en gráficas generales este problema es \textit{NP-completo}, pero en gráficas bipartitas como esta resulta que la clave para resolver el problema es usar flujo máximo.
\\\\Esto puede parecer raro porque la gráfica que construimos no parece indicar por ningún lado un flujo, pero podemos construir la red agregando un vértice $s$ conectado a cada fila y un vértice $t$ conectado a cada columna. A todas las aristas les pondremos capacidad $1$.
\begin{figure}[H]
\centering
\begin{tikzpicture}[
  every node/.style={circle,draw,minimum size=0.65cm,font=\small},
...
\begin{definition}Un \textbf{emparejamiento} es un subconjunto de aristas tal que ningún vértice es extremo de más de una arista. El \textbf{emparejamiento máximo} es el de mayor cardinalidad.
\end{definition}
 
Traducido al problema que estamos resolviendo, un emparejamiento en nuestra gráfica es un conjunto de enemigos tal que no hay dos en la misma fila ni en la misma columna. El emparejamiento máximo es el mayor conjunto así.
...
\begin{tikzpicture}[
  ...
  \node[draw=none,font=\small,teal] at (2,-0.8) {emparejamiento máximo = 3};
\end{tikzpicture}
\end{figure}

\subsection*{Hopcroft-Karp: Dinic en gráficas bipartitas}
 
El emparejamiento máximo en una gráfica bipartita se puede calcular directamente
...
optimizar, el algoritmo \textbf{Hopcroft-Karp} es básicamente Dinic
aplicado a gráficas bipartitas, con complejidad $O(E\sqrt{V})$ en lugar
de $O(V^2 E)$.
...
// Intenta aumentar el emparejamiento desde u siguiendo las capas de BFS.
bool dfs(int u) {
    for (int v : adj[u]) {
        int w = matchR[v];
...
El \textbf{teorema de König} nos dice que en una gráfica bipartita la cardinalidad del emparejamiento máximo es igual a la cardinalidad de la cubierta mínima de vértices\footnote{\url{https://www.math.nagoya-u.ac.jp/~richard/teaching/s2024/SML_Li_2.pdf}}.
\\\\
Veamos la red del problema de los cañones. El flujo máximo equivale al emparejamiento
máximo (cada unidad de flujo es un par emparejado). El corte mínimo corresponde a la cubierta mínima; es decir, dado el corte mínimo que es una partición ($S,T$) de los vértices tal que $s\in S$ y $t \in T$, la cubierta mínima se obtiene tomando las filas que quedaron del lado de $T$ y las columnas que quedaron del lado de $S$. Estas son las filas/columnas que hay que disparar. Por el teorema anterior, el tamaño de esta cubierta coincide con el enparejamiento máximo. Entonces calculado el flujo máximo, ¿cómo construir la cubierta mínima?
...
Podemos usar Hopcroft-Karp o Dinic para encontrar el emparejamiento máximo de tamaño 3 con las aristas:
$f_1\text{-}c_1,\ f_2\text{-}c_2,\ f_3\text{-}c_4$.
\\\\
Lo siguiente que haremos, será partir desde cada vértice libre del lado izquierdo, en este caso solo tenemos $f_4$ y alternar aristas fuera/dentro del emparejamiento. En la figura de abajo, usamos $f_4\to c_2$ que está fuera del emparejamiento, luego $c_2\to f_2$ que está dentro del emparejamiento y ya no tenemos a dónde movernos pues $c_2\to f_2$ ya fue visitada.
...
Ahora, para construir la cubierta usaremos estos vértices que marcamos (los que quedaron en amarillo), es decir, los vértices alcanzables desde $s$ en la gráfica residual tras calcular el flujo máximo. La cubierta mínima se forma con:
\begin{itemize}
...
Una pregunta natural que podría surgir es ¿cuándo existe un emparejamiento que empareja a \textbf{todos} los vértices de un lado?
\\\\
En una gráfica bipartita con lados $L$ y $R$, existe un emparejamiento que
...
\begin{tikzpicture}[
  ...
  \begin{scope}
    ...
    \node[draw=none,font=\small] at (1.5,-0.9) {emparejamiento perfecto};
  \end{scope}
  ...
  \begin{scope}[xshift=6cm]
    ...
    \node[draw=none,font=\small] at (1.5,-0.9) {emparejamiento perfecto existe};
  \end{scope}
\end{tikzpicture}
\end{figure}
\subsection*{Emparejamiento de peso mínimo}
Cuando las aristas tienen pesos y queremos el emparejamiento perfecto de costo
...
$i$ a la tarea $j$, minimizar el costo total. En caso de que al lector le interese aprender más de este tema, un buen recurso para aprender este algoritmo es \url{https://www.topcoder.com/thrive/articles/Assignment%20Problem%20and%20Hungarian%20Algorithm}.
\newpage
\section{Ejercicios}
...
\subsection*{9.10.1 \href{https://codeforces.com/problemset/problem/808/F}{Card Game (Codeforces 808F)}}
...
\vspace{0.5em}
\textbf{Input}\\
The first line contains two integers $n$ and $k$ ($1 \leq n \leq 100$, $1 \leq k \leq 100000$).

Then $n$ lines follow, each of these lines contains three numbers that represent the corresponding card: $p_i$, $c_i$ and $l_i$ ($1 \leq p_i \leq 1000$, $1 \leq c_i \leq 100000$, $1 \leq l_i \leq n$).
...
sugiere separar las cartas en dos grupos: pares e impares. Así, las aristas prohibidas solo conectarían cartas de grupos distintos, dando una gráfica bipartita.
...
Sean $A$ las cartas con $c_i$ impar, y $B$ las cartas con $c_i$ par disponibles con nivel $L$. Entonces, construiremos nuestra gráfica poniendo flechas como sigue: 

\begin{itemize}
    \item Una flecha desde la fuente a cada carta $i$ en $A$, con capacidad $p_i$.
    \item Una flecha desde cada carta $j$ en $B$ hacia el sumidero, con capacidad $p_j$.
    \item Una flecha de $i$ a $j$ (impar a par), con capacidad $\infty$, si $c_i+c_j$ es primo.
\end{itemize}
...
La complejidad es $O(\log n)$ para las iteraciones de búsqueda binaria, cada una con un max flow en una red de $O(n)$ vértices y $O(n^2)$ aristas.
\\\\
Ver \codelink{https://github.com/MelitoMT/ICPC_Manual/blob/master/tex/codes/editorial/808F.cpp}.

\subsection*{9.10.2 \href{https://www.spoj.com/problems/QTREE2/}{Query on a tree II (SPOJ)}}

\begin{center}
\fbox{
\begin{minipage}{0.95\linewidth}
\begin{center}
\textbf{QTREE2 - Query on a tree II}\\
\textbf{time limit:} 1 s\\
\textbf{memory limit:} 1536 MB
\end{center}

You are given a tree (an undirected acyclic connected graph) with $N$ nodes, and edges numbered $1, 2, 3 \dots N-1$. Each edge has an integer value assigned to it, representing its length.

We will ask you to perform some instructions of the following form:
\begin{itemize}
    \item \texttt{DIST a b}: ask for the distance between node $a$ and node $b$, or
    \item \texttt{KTH a b k}: ask for the $k$-th node on the path from node $a$ to node $b$.
\end{itemize}

For example, if $N=6$ and the edges are \texttt{1 2 1}, \texttt{2 4 1}, \texttt{2 5 2}, \texttt{1 3 1}, \texttt{3 6 2} (an edge \texttt{a b c} connects node $a$ and node $b$ with cost $c$), the path from node $4$ to node $6$ is $4 \to 2 \to 1 \to 3 \to 6$. Then \texttt{DIST 4 6} is $5$ ($1+1+1+2=5$) and \texttt{KTH 4 6 4} is $3$ (the $4$-th node on the path from node $4$ to node $6$ is $3$).
...
For each test case:
\begin{itemize}
    \item In the first line there is an integer $N$ ($N \leq 10000$).
    \item In the next $N-1$ lines, the $i$-th line describes the $i$-th edge: a line with three integers \texttt{a b c} denotes an edge between $a$, $b$ of cost $c$ ($c \leq 100000$).
    \item The next lines contain instructions \texttt{DIST a b} or \texttt{KTH a b k}.
    \item The end of each test case is signified by the string \texttt{DONE}.
\end{itemize}
...
\vspace{0.5em}
\textbf{Output}\\
For each \texttt{DIST} or \texttt{KTH} operation, write one integer representing its result.
...
\begin{itemize}
    ...
    \item En caso contrario:
          ...
          El $k$-ésimo vértice está en el camino
          $L \to b$. El vértice está a:
          \[\text{s} = dist(a,b)-(k-1)\]
...
\end{itemize}
 ...
 Ver \codelink{https://github.com/MelitoMT/ICPC_Manual/blob/master/tex/codes/editorial/qtree2.cpp}.
